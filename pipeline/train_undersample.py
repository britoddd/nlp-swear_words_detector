"""
Strategy: Random undersampling on TF-IDF features.
Down-samples all classes to match the smallest class (Level 3).
"""
...
def apply_undersample(X, y):
    ...
    logger.info(f'Undersampling applied | total={len(y_res):,}')
    return X_res, y_res


if __name__ == '__main__':
    log_path = setup_logging('classical_undersample.log')
    logger.info('=' * 60)
    logger.info('Strategy: random undersampling on TF-IDF features')

    X_train, X_test, y_train, y_test = load_splits()
    tfidf_lr, tfidf_nb, X_train_lr, X_test_lr, X_train_nb, X_test_nb = \
        build_tfidf_features(X_train, X_test)

    X_train_lr_s, y_train_lr_s = apply_undersample(X_train_lr, y_train)
    X_train_nb_s, y_train_nb_s = apply_undersample(X_train_nb, y_train)
